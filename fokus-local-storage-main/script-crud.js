...
function criarElementosLi (tarefa) {
    ...
    botao.onclick = () => {
        const novoNomeTarefa = prompt("Digite o novo nome para a tarefa:")
        console.log("Novo nome da tarefa:", novoNomeTarefa)
        if (novoNomeTarefa) {
            paragrafo.textContent = novoNomeTarefa
            tarefa.descricao = novoNomeTarefa
            atualizarTarefas()    
        } 
...
adicionarTarefaForm.addEventListener('submit', (evento) => {
    ...
    tarefas.push(tarefa)
    atualizarTarefas()
    const elementoCriado = criarElementosLi(tarefa)
